...
s = (s==0) + s; % s(i)== 1 si b(i) >=0 , s(i)== -1 si b(i) < 0
A = repmat(s,1,n).*A;
b(s==-1) = -b(s==-1);
...
while any(OPT*pentes > 0) % Y-a-t-il un chemin améliorant ?
    ...
    if any(not(any(A(JNP) > 0))) || any(OPT*A(JNP) <= 0) % on peut écrire any et not dans n'importe quel sens
    ...
    % GAMMA(indbeta,alpha) est le pivot de l'itération
    %
    GAMMA(indbeta,:) = GAMMA(indbeta,:)/GAMMA(indbeta,alpha);
    GAMMAMOINS = GAMMA(indbeta+1:size(GAMMA,1),:);
    GAMMAMOINS = GAMMAMOINS - GAMMAMOINS(:,alpha)*GAMMA(indbeta,:);
    GAMMA(indbeta+1:size(GAMMA,1),:) = GAMMAMOINS;
