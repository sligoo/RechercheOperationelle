...
while t <= maxtempsparcours % Boucle temporelle avec maxtempsparcours variable à cause des collisions éventuelles
    ...
    for k = 1:nombre_objets
        if t < tempsparcours(k)
            % Si le robot 1 en détecte un autre (sauf au tout premier tour)
            if k == 1 && ~all(detection == false) && t > 1
              % On stocke la prochaine position du robot 1
              ii1 = round(P(1).pp(t - tRetard));
              % Pour chaque robot l > 1
              for l = 2:nombre_objets
                % Si l est toujours en chemin
                if t <= size(P(l).pp, 2)
                  % On stocke la prochaine position de l
                  iil = round(P(l).pp(t));
                  % Si les deux robots doivent se rencontrer au prochain tour
                  if abs(xy(1, ii1, 1) - xy(1, iil, l)) < T(1).diametre_robot + T(l).diametre_robot && abs(xy(2, ii1, 1) - xy(2, iil, l)) < T(1).diametre_robot/2 + T(l).diametre_robot/2
                    % On compte un tour de retard (le robot s'arrête pour un tour)
                    tRetard = tRetard + 1;
                    tempsparcours(1) = tempsparcours(1) + 1;
                    maxtempsparcours = max(tempsparcours);
                    break
                  end
                end
            ...
            % Pour le robot 1, on prend en compte le nombre total de tours de retard
            if k == 1
              ii(k) = round(P(k).pp(t - tRetard));
